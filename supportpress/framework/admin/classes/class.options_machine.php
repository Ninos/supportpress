	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					if(isset($itdata[$value['id']])) $t_value = stripslashes($itdata[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$select_ID.'" ' . selected($itdata[$value['id']], $select_ID, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($itdata[$value['id']]);			
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($itdata[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($itdata[$value['id']])) {
						$itdata[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($itdata[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					if (isset($itdata[$value['id']])) $multi_stored = $itdata[$value['id']];
				...
				//colorpicker option
				case 'color':		
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$itdata[$value['id']].'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $itdata[$value['id']] .'" />';
				break;
				
				//typography option	
				case 'typography':
				
					if (isset($itdata[$value['id']]) ) $typography_stored = $itdata[$value['id']];
				...
				case 'images':
					...
					$select_value = '';
					if (isset($itdata[$value['id']])) $select_value = $itdata[$value['id']];
				...
				case 'sorter':
					...
					if (isset($itdata[$value['id']])) $sortlists = $itdata[$value['id']];
