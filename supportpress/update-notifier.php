...
* Profile: http://themeforest.net/user/icaruscreativeorg *
* Follow me: http://twitter.com/icaruscreativeorg *
* *
**************************************************************/
global $itdata;
...
define( 'NOTIFIER_XML_FILE', 'http://www.icaruscreative.org/pressthemes/updates/projectpress.xml' ); // The remote notifier XML file containing the latest version of the theme and change log
define( 'NOTIFIER_CACHE_INTERVAL', 21600 ); // The time interval for the remote XML cache in the database (21600 seconds = 6 hours)
define ('PRTH_UPDATE_PAGE_NAME', 'theme-update-notifier');
...
function update_notifier_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string function isn't available
		...
		$count = (isset($_GET['prth_update']) &&  $_GET['prth_update']=='true')?'':'<span class="update-plugins count-1"><span class="update-count">1</span></span>';
		if($newversion) { // Compare current theme version with the remote XML version
			add_theme_page( NOTIFIER_THEME_NAME . ' Theme Updates', NOTIFIER_THEME_NAME . ' Update '.$count, 'administrator', prth_UPDATE_PAGE_NAME, 'update_notifier');
		} 
	}
}
...
function update_notifier() {
	$xml = get_latest_theme_version(NOTIFIER_CACHE_INTERVAL); // Get the latest remote XML file on our server
	$theme_data = wp_get_themes(TEMPLATEPATH . '/style.css'); // Read theme current version from the style.css
	$options_url= admin_url('index.php?page='.prth_UPDATE_PAGE_NAME.'&prth_update=true'); ?>

	<script type="text/javascript">
	var ppUpdateData = {
		optionsLink: "<?php echo $options_url; ?>",
		envatoDetails: <?php if (isset($itdata['_tf_username']) && isset($itdata['_tf_api_key'])) { echo "true"; } else { echo "false" ; } ?>
	};
	</script>
	...
	<?php if(!(isset($_GET['prth_update']) &&  $_GET['prth_update']=='true')){ ?>
...
function get_latest_theme_version($interval) {
	...
	if( strpos((string)$notifier_data, '<notifier>') === false ) {
		$notifier_data = '<?xml version="1.0" encoding="UTF-8"?><notifier><latest>2.1.0</latest><changelog></changelog></notifier>';
	}
...
add_action('admin_init', 'prth_set_update_functionality');

function prth_set_update_functionality(){
	// include the library
	...
	if (isset($itdata['_tf_username'])) { $tf_username = $itdata['_tf_username']; } else $tf_username = '';
	if (isset($itdata['_tf_api_key'])) { $tf_api_key = $itdata['_tf_api_key']; } else $tf_username = '';
	$allow_cache=true;
	...
	if($tf_username && $tf_api_key){
		...
		if(isset($_GET['prth_update']) &&  $_GET['prth_update']=='true'){
			if (in_array  ('curl', get_loaded_extensions())){
				...
				$success = $res->success;
				$prth_data->theme_updated = $success;
			}else{
				$prth_data->curl_disabled = true;
			}
		}
	}
}
...
add_action('admin_notices', 'prth_update_notice' );

function prth_update_notice(){
	global $prth_data;
	$message_type="updated";
	
	if(isset($prth_data->theme_updated)){
		if($prth_data->theme_updated){
	...
	}elseif(isset($prth_data->curl_disabled) && $prth_data->curl_disabled){
